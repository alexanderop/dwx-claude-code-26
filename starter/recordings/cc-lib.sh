...
# call `cc_banner` in the tape's hidden setup. Every Claude Code clip then opens
# with the identical mascot + header, and the brand palette lives in ONE place.
#
#   source ./cc-lib.sh
#   ...your shim body, using $PINK/$DIM/$BOLD/... as before...
#
# (vp-sim.sh is intentionally NOT a consumer — it's the Vite+ CLI, not Claude.)
...
E="${DIM}□${RST} "  # empty context-grid cell (used by context-sim.sh)
...
# cc_banner [cwd] [mcp]
#   cwd  — working-dir line shown under the model  (default: ~/workout-tracker)
...
# Sets $CC_INPUT to whatever was typed. Replaces the hand-rolled prompt+read that
# goal-sim.sh / subtree-sim.sh each duplicated.
cc_prompt() {
  printf "${PINK}❯${RST} "
  read -r CC_INPUT
  printf "\n"
